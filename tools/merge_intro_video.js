...
async function main() {
  const unzippedDir = 'C:\\Users\\Vov\\.gemini\\antigravity\\scratch\\temp_unzip';
  ...
  const files = fs.readdirSync(unzippedDir)
  ...
  const concatListPath = path.join(__dirname, '..', 'temp_concat_list.txt');
  ...
  const outputFileName = 'flappy_intro_scene_40s.mp4';
