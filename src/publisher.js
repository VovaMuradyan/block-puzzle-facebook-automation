...
async function runPublisher() {
  ...
  for (const page of pages) {
    ...
    // Rule 1: Strictly 1 post per page every 15 minutes
    if (!canPostToPage(pageState, 15)) {
      const lastPostTime = pageState.last_post_at ? new Date(pageState.last_post_at).toLocaleTimeString() : 'Never';
      console.log(`[Publisher] SKIPPED ${pageName}: Last posted at ${lastPostTime}. Must wait 15 minutes between posts.`);
      continue;
    }
